...
function loadPostList(page) {
    var data="";
    if (null != searcherMap){
        var southWest = searcherMap.getBounds().getSouthWest();
        var northEast = searcherMap.getBounds().getNorthEast();
        data += "postLatStart=" + southWest.lat + "&postLatEnd=" + northEast.lat + "&postLngStart=" + southWest.lng + "&postLngEnd=" + northEast.lng ;
    }

    data += "&page=" + $.trim(page)
    var request = {
        url: "/post/lost/post/queryListPost",
        data:data,
